...
object GameJson {
  ...
  implicit val drawSourceReads: Reads[DrawSource] = Reads {
    ...
    case c: Card          => JsSuccess(PileSource(c))
  ...
  implicit val cardReads: Reads[Card] = Reads {
    ...
    case e => JsError("Not a valid card id: $e")
  }

  case class JsonCard(id: String, gameString: String, endValue: Int)
      extends Card

  implicit val jsonCardWrites: OWrites[JsonCard] = Json.writes[JsonCard]

  implicit val cardWrites: Writes[Card] = { card =>
    Json.toJsObject(JsonCard(card.id, card.gameString, card.endValue))
  }
  implicit val drawableCardReads: Reads[DrawableCard] = Json.reads[DrawableCard]
  implicit val drawableCardWrites: Writes[DrawableCard] =
    Json.writes[DrawableCard]
  ...
  implicit val gameStateViewWrites: OWrites[GameStateView] =
    Json.writes[GameStateView]
}
